...
```{r turn-off-eval, echo=FALSE, eval=TRUE}
knitr::opts_chunk$set(eval = FALSE, fig.show = "hide")
```
...
```{r plot-transport-raw, fig.show='asis', echo=FALSE}
ggplot(transport, aes(x = weight, y = height)) +
  geom_point()
```
...
Now set the shape of each point by the gender of our study participant
...
```{r plot-transport-colour-shape, echo=FALSE, eval=TRUE, fig.show='asis'}
ggplot(
  transport, 
  aes(x = weight, y = height, colour = method, shape = gender)
) +
...
- Top-level aesthetics are passed down through all layers
- Aesthetics can be set to respond to variables (i.e. columns)
- Can also be provided as fixed values
    + Fixed values are usually specific to a layer
...
```{r plot-transport-with-lm, echo=FALSE, fig.show='asis'}
ggplot(transport, aes(x = weight, y = height)) +
  geom_point(aes(colour = method, shape = gender)) +
  geom_smooth(method = "lm", formula = y~x, se = FALSE)
```
...
- If we had left `colour = method` in the original `aes()` we would produce two lines
...
- If we'd also left `shape` assigned in the initial call
    ...
    + Could add a `linetype` or `linewidth` aesthetic $\implies$ looks terrible
